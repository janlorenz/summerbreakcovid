...
download.file(
  "https://github.com/robert-koch-institut/SARS-CoV-2-Infektionen_in_Deutschland/raw/main/Aktuell_Deutschland_SarsCov2_Infektionen.csv",
  destfile = "data/Aktuell_Deutschland_SarsCov2_Infektionen.csv"
)
RKIraw <- read_csv(
  "data/Aktuell_Deutschland_SarsCov2_Infektionen.csv",
...
get_summervac <- function(year) {
  read_csv2(paste0("data/", year - 1, "-", year, ".csv")) |>
    select(Land, Sommer) |>
    separate(Sommer, into = c("start", "end"), sep = " [–-] ") |>
    mutate(
      start = paste0(start, year) |> lubridate::dmy(),
